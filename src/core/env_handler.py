...
class EnvironmentManager:
    ...
    def is_exclusive_venv(self) -> bool:
        """
        Check if we are running in a project-specific .venv_* or allowed Conda env.
        """
        in_venv = sys.prefix != sys.base_prefix
        current_prefix = Path(sys.prefix).resolve()
        
        # Check if current environment is a project venv (.venv_*)
        # We look for folders in project_root starting with .venv
        try:
            if current_prefix.is_relative_to(self.project_root.resolve()):
                rel_parts = current_prefix.relative_to(self.project_root.resolve()).parts
                if rel_parts and rel_parts[0].startswith(".venv"):
                    return True
        except (ValueError, Exception):
            pass
            
        # Fallback for active Conda environments
        if self.is_conda():
            return True

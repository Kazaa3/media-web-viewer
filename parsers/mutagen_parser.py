    ...
    try:
        if file_type == '.flac':
            ...
            if PARSER_CONFIG.get('mutagen_prefer_albumartist'):
                tags['artist'] = safe_get(audio_flac, 'ALBUMARTIST') or safe_get(audio_flac, 'ARTIST', default=tags.get('artist', 'Unbekannt'))
            else:
            ...
            chapter_keys = [k for k in audio_flac.keys() if k.startswith('CHAPTER') and not k.endswith('NAME') and not k.endswith('URL')]
            ...
            if PARSER_CONFIG.get('mutagen_prefer_albumartist'):
                tags['artist'] = safe_get(audio_mp4, 'aART') or safe_get(audio_mp4, '\xa9ART', default=tags.get('artist', 'Unbekannt'))
            else:
            ...
            audio_misc: Any = None
            if file_type == '.ogg': audio_misc = OggVorbis(path)
            elif file_type == '.opus': audio_misc = OggOpus(path)
            elif file_type == '.wav': audio_misc = WAVE(path)
            elif file_type == '.aac': audio_misc = AAC(path)
            elif file_type == '.wma': audio_misc = ASF(path)

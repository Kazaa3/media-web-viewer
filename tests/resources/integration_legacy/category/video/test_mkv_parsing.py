...
class TestMKVParsing:
    ...
    @patch('src.core.models.MediaItem.extract_artwork', return_value=None)
    @patch('src.parsers.media_parser.extract_metadata')
    ...
    @patch('src.parsers.artwork_extractor.ArtworkExtractor._run_ffmpeg', return_value=True)
    @patch('src.parsers.media_parser.extract_metadata')
    def test_mkv_artwork_extraction_flow(self, mock_extract, mock_ffmpeg):
        ...
        with patch('src.parsers.artwork_extractor.Path.exists', side_effect=[True, False, True, True]), \
             patch('src.parsers.artwork_extractor.Path.mkdir'), \
             patch('src.parsers.artwork_extractor.Path.stat') as mock_stat:

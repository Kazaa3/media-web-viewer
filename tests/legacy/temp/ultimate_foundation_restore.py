...
# EXACT Mapping for the 01-13 History
GAPLESS_SERIES = [
    ("The Skeleton", "001"),
...
def absolute_restoration_v4():
    all_files = []
    # Collect from all 3 folders
    for folder in ["01_Architektur_und_Konzepte", "02_Features_und_Implementation", "03_Walkthroughs_und_Berichte"]:
        ...
        for f in f_dir.rglob("*.md"):
            if f.is_file():
                ...
                # Check for story matches, but AVOID summaries
                forced_id = None
                # If it contains "22.03.2026", it's a summary of today!
                is_summary = "22.03.2026" in content or "logbuch: fixes" in content or "master overhaul" in content
    ...
    # Sort others: summaries come at the end of the history
    # Sort by mtime initially, but summaries will be pushed later
    others.sort(key=lambda x: (x["is_summary"], os.path.getmtime(x["path"])))
    ...
    print(f"Executing ULTIMATE Foundation Restoration (001-013) and Re-indexing {len(final_list)} records...")
    for entry, idx in final_list:
        new_name = f"{idx}_{entry['clean_name']}"
        # Story (0-25) always in Architecture

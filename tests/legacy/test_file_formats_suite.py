...
class TestFileFormatsReal(unittest.TestCase):
    ...
    def test_video_formats_detection_real(self):
        """[REAL CONTAINER TEST] Scans real 1-second video files generated by FFmpeg."""
        import subprocess
        SCRIPT_PATH = Path(__file__).parent / "scripts" / "generate_test_media.py"
        ARTIFACTS_DIR = Path(__file__).parent / "artifacts" / "real_media"
        
        # Ensure media exists
        if not ARTIFACTS_DIR.exists() or not list(ARTIFACTS_DIR.glob("*.mp4")):
            subprocess.run(["python3", str(SCRIPT_PATH)], check=True)
            
        real_media_dir = os.path.join(self.test_dir, "real_media")
        os.makedirs(real_media_dir, exist_ok=True)
        
        # Copy real files to test temp dir
        copied_files = 0
        for f in ARTIFACTS_DIR.glob("*"):
            if f.is_file():
                shutil.copy2(f, os.path.join(real_media_dir, f.name))
                copied_files += 1
                
        if copied_files == 0:
            self.skipTest("No real media artifacts generated.")
            
        main.scan_media(dir_path=real_media_dir, clear_db=True)
        # Verify scanner inserted them
        # test_video.mp4, .mkv, .webm, .avi, .ts, test_dvd.iso
        self.assertGreaterEqual(self.mock_insert.call_count, 5)

...
class TestMissingCoverCategory:
    ...
    @patch('src.parsers.artwork_extractor.extractor.extract')
    def test_missing_cover_flag(self, mock_extract_art, mock_metadata):
        ...
        with patch('src.core.models.Path.exists', return_value=True), \
             patch('src.core.models.Path.is_dir', return_value=False), \
             patch('src.core.models.Path.stat'):

...
def test_version_consistency():
    ...
    # Read control file
    control_path = Path(__file__).parents[3] / "infra" / "packaging" / "DEBIAN" / "control"
...
def test_deb_build_script_exists():
    ...
    script_path = Path(__file__).parents[3] / "infra" / "build_deb.sh"
...
def test_installed_version_matches_current_build():
    ...
    installed_state = f"{parts[0]} {parts[1]} {parts[2]}"
...
def test_destructive_reinstall_e2e_optional():
    ...
    installed_state = f"{output[0]} {output[1]} {output[2]}"
...
def test_purge_removes_everything():
    """Verify that apt purge removes the entire app directory."""
    print("\n🧪 Test 9: Purge Verification (optional)")
    ...
    run_command(f"sudo apt-get purge -y {package_name}", check=False)
    
    if os.path.exists(app_dir):
        print(f"❌ Error: {app_dir} still exists after purge!")
        try:
            content = os.listdir(app_dir)
            print(f"   Remaining files: {content}")
        except Exception:
            pass
        return False
    
    print(f"✅ Purge successful: {app_dir} removed.")
    return True

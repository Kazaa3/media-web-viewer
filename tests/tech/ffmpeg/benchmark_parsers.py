# Kategorie: Performance Test
# Eingabewerte: Reale Medien im /media Ordner
# Ausgabewerte: Ausführungszeiten (STDOUT)
# Testdateien: /media/*
# Kommentar: Misst die Geschwindigkeit von pymediainfo, mutagen, ffmpeg etc.

from typing import Any
import time
import glob
from pathlib import Path
import sys
import os

PROJECT_ROOT = os.path.abspath(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../..')))
if PROJECT_ROOT not in sys.path:
    

from src.parsers import filename_parser, mutagen_parser, pymediainfo_parser, ffprobe_parser, ffmpeg_parser, isoparser_parser

media_dir = os.path.join(PROJECT_ROOT, 'media')
files = glob.glob(os.path.join(media_dir, '*.*'))

parser_times = {
    "filename": 0.0,
    "container": 0.0,
    "pymediainfo": 0.0,
    "ffprobe": 0.0,
    "ffmpeg": 0.0,
    "mutagen": 0.0,
    "isoparser": 0.0,
    "ebml": 0.0,
    "enzyme": 0.0,
    "pycdlib": 0.0,
    "pymkv": 0.0
}

for f in files:
    name = os.path.basename(f)
    path_obj = Path(f)
    file_type = path_obj.suffix.lower()

    # filename parser
    t0 = time.time()
    tags_in: dict[str, Any] = {}
    tags_res = filename_parser.parse(path_obj, file_type, tags_in, name)
    parser_times["filename"] += (time.time() - t0)

    # mutagen parser
    t0 = time.time()
    tags_res = mutagen_parser.parse(path_obj, file_type, {}, name)
    parser_times["mutagen"] += (time.time() - t0)

    # pymediainfo parser
    t0 = time.time()
    tags_res = pymediainfo_parser.parse(path_obj, file_type, {}, name)
    parser_times["pymediainfo"] += (time.time() - t0)

    # ffprobe parser
    t0 = time.time()
    tags_res = ffprobe_parser.parse(path_obj, file_type, {}, name)
    parser_times["ffprobe"] += (time.time() - t0)

    # ffmpeg parser
    t0 = time.time()
    tags_res = ffmpeg_parser.parse(path_obj, file_type, {}, name)
    parser_times["ffmpeg"] += (time.time() - t0)

    # isoparser parser
    t0 = time.time()
    if file_type == ".iso":
        tags_res = isoparser_parser.parse(path_obj, file_type, {}, name)
    parser_times["isoparser"] += (time.time() - t0)

    # ebml parser (python-ebml)
    t0 = time.time()
    if file_type == ".mkv":
        try:
            from ebml.container import File
            ebml_file = File(str(path_obj))
            segment = next(ebml_file.children_named("Segment"), None)
        except Exception:
            pass
    parser_times["ebml"] += (time.time() - t0)

    # enzyme parser
    t0 = time.time()
    if file_type in [".mkv", ".mp4"]:
        try:
            import enzyme
            movie = enzyme.Movie(str(path_obj))
        except Exception:
            pass
    parser_times["enzyme"] += (time.time() - t0)

    # pycdlib parser
    t0 = time.time()
    if file_type == ".iso":
        try:
            import pycdlib
            iso = pycdlib.PyCdlib()
            iso.open(str(path_obj))
            iso.close()
        except Exception:
            pass
    parser_times["pycdlib"] += (time.time() - t0)

    # pymkv parser
    t0 = time.time()
    if file_type == ".mkv":
        try:
            import pymkv
            mkv = pymkv.MKVFile(str(path_obj))
        except Exception:
            pass
    parser_times["pymkv"] += (time.time() - t0)

    # container parser
    t0 = time.time()
    tags_container: dict[str, Any] = {}
    if not tags_container.get('container'):
        tags_container['container'] = file_type[1:].lower()
        if not tags_container.get('codec'):
            tags_container['codec'] = file_type[1:].lower()
    parser_times["container"] += (time.time() - t0)

print(f"Total files: {len(files)}")
print("-" * 30)
for parser, t in parser_times.items():
    print(f"{parser:<15}: {t:.6f} seconds")
print("-" * 30)

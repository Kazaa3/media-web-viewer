...
Wir haben die Test- und Build-Pipeline auf professionelle Standards gehoben und Lücken bei Windows-Support und Test-Transparenz geschlossen.

1. Windows Build Isolation
- build_exe.sh refaktoriert: Windows-Executables werden ins root build/-Verzeichnis exportiert.
- Isoliertes Staging: Fragmente werden strikt im build/-Ordner bereinigt und verwaltet.

2. Structured Test Reporting
- build_system.py unterstützt das --report-Flag für alle Test-Tiers.
...
Wir haben die Zuverlässigkeit und Beobachtbarkeit des Build-Systems deutlich verbessert.

1. Performance Measurement
- Build-System misst die Ausführungszeit auf mehreren Ebenen:
  - Phase Timing: Clean, Tests, Build werden im Timer-Kontext ausgeführt.
  - Subprocess Timing: Jeder _run_command wird getimed.
  - Build Session Summary: Am Ende jeder Ausführung wird ein Report mit "Total Duration" und "Phase Durations" ausgegeben.

2. Resilience and Hang Prevention
- Subprocess Timeouts: Alle Standard-Kommandos haben jetzt einen Default-Timeout von 600s.
- Robust Monitoring: monitor_utils.py überarbeitet für zuverlässige Hang-Erkennung basierend auf Output-Inaktivität.
- Graceful Abortion: Bei Hang wird der gesamte Prozessbaum beendet, um Zombie-Prozesse zu verhindern.
